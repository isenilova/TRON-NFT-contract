...
/// * 
/// */
contract MintOTL is TRC721, TRC721Enumerable, TRC721MetadataMintable {
    ...
    string public message;
        ...
        constructor() public TRC721Metadata("OneThatLife", "OTL") {
     _setBaseURI('https://gateway.btfs.io/btfs/Qmf85hMg3NRBgxWDSy4YfQbdgJRWXFRHQ2P3WSazDjxZR2');
     }



    function safePublicMint(address _to, uint256[] memory _tokenId) public onlyOwner {
      require(_to != address(0x0), "Address error");
     
      require(_tokenId.length > 0, "Input error");
     

      for (uint i = 0; i < _tokenId.length; i++) {
        mintWithTokenURI( _to, _tokenId[i], _tokenId[0].toString() );
      }
    }
    
     function publicOTLMint(address _to, string memory newMessage) public {
      require(_to != address(0x0), "Address error");
     ...
     /// for (uint i = 0; i < _tokenId.length; i++) {
         ...
         _to = msg.sender;
        mintWithTokenURI( _to, _currentId, _currentId.toString() );
        _currentId ++;
        message = newMessage;
     /// }
    }
    ...
    function preMint(address _to, uint256[] memory _tokenId) public onlyOwner {
      require(_to != address(0x0), "Minting to the null address is not allowed");
        ...
        mintWithTokenURI( _to, _tokenId[0], _tokenId[0].toString() );
